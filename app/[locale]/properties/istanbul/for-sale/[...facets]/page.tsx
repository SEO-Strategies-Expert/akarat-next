...
const TYPE_LABELS_IST: Record<string, Record<string, string>> = {
...
export default async function IstanbulFacetPage({ params }: Params) {
  ...
  const typeLabels = TYPE_LABELS_IST[locale] ?? TYPE_LABELS_IST.en;
  ...
  try {
    const [propertiesData, cities, cats] = await Promise.all([
    ...
    if (parsed.city) {
      ...
      if (cityId) properties = properties.filter((p) => String(p.city_id) === cityId);
  ...
  const basePath = locale === 'ar' ? '/properties/istanbul/for-sale' : `/${locale}/properties/istanbul/for-sale`;
